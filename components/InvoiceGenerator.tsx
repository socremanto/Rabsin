...
const generateInvoiceNumber = (quoteRef: string) => {
  const parts = quoteRef.split('/');
  return `INV-${parts[0].slice(-4)}-${parts[1]}`;
};
...
export function InvoiceGenerator({
  ...
  const [invoiceNumber, setInvoiceNumber] = useState(generateInvoiceNumber(quoteReference))
  ...
  const generateInvoice = () => {
    ...
    // Add title
    doc.setFontSize(18);
    doc.text('Invoice', margin + 30, margin + 10);

    // Add company info and date/invoice reference
    doc.setFontSize(10);
    ...
    doc.text(`Date: ${invoiceDate}`, pageWidth - margin - 60, margin + 30);
    doc.text(`Invoice No: ${invoiceNumber}`, pageWidth - margin - 60, margin + 35);
    doc.text(`Quote Ref: ${quoteReference}`, pageWidth - margin - 60, margin + 40);
    ...
    const newText = "This invoice is system generated and does not require any stamp or signature. If you have any questions concerning this invoice, please contact us by email: info@palmdt.com";
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <h3 className="font-semibold mb-2">Invoice Details (from Quotation)</h3>
      ...
      <Button onClick={generateInvoice}>Generate and Save Invoice</Button>
    </div>
  )
}
